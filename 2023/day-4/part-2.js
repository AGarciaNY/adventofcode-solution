...
for (let i = 0; i < sortedData.length; i++) {
    ...
    for (let j = 0; j < myNumers.length; j++) {
        if (luckyNumberSet.has(myNumers[j])) {
            cardWon++
        };
    };
    console.log(cardWon)

    for (let j = i + 2; j < cardWon + i + 2; j++) {
        checkList.push(j)
        if (cardWonScratch[i + 1]) {
            cardWonScratch[i + 1].push(j)
        } else {
            cardWonScratch[i + 1] = [j]
        }
    }
}


console.log(cardWonScratch, count,checkList)